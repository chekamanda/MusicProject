...
public class ColorDisplay {
	...
	public static void openDisplay(String title)
	{
		...
		for (int c = 0; c < H_MAX_PIX; c++)
			...
			{
				screen[r][c] = new Pixel(r, c, ROW_LABEL_WIDTH + c * PIXEL_SIZE,
						(V_MAX_PIX - 1) * PIXEL_SIZE - r * PIXEL_SIZE, PIXEL_SIZE, PIXEL_SIZE, PIXEL_SIZE,
						scalingFactor);
		...
		for (int r = 0; r < V_MAX_PIX; r++)
		{
			rowLabels[r] = new RowLabel(r, 0, (V_MAX_PIX - 1) * PIXEL_SIZE - r * PIXEL_SIZE, ROW_LABEL_WIDTH,
					PIXEL_SIZE, scalingFactor);
		...
		for (int c = 0; c < H_MAX_PIX; c++)
		{
			colLabels[c] = new ColLabel(c, ROW_LABEL_WIDTH + c * PIXEL_SIZE, V_MAX_PIX * PIXEL_SIZE, PIXEL_SIZE,
					COL_LABEL_HEIGHT, scalingFactor);
	...
	private static Color pickColor(String color)
	{
		...
		if (color.equals("RED"))
			...
			result = Color.gray;
		else if (color.equals("WHITE")) // addon
			result = Color.white;
		else if (color.equals("PINK")) // addon
			result = Color.pink;
		else if (color.equals("YELLOW")) // addon
			result = Color.yellow;
